# HANDOFF — ohanashi（お話の記憶アプリ + PRO）

## 0-AB. 最新状態（2026-07-29・20観点監査 → 18件修正 **本番反映済み** `1b7d75e`）

**本番 https://ohanashi.bizsp.net/pro/play は 244話・詰みなし・全端末でボタンに到達可・課金APIは停止（503）。**

### この日の起点: 泰介さんの報告「何問か解いたら、途中で次の問題に行くボタンがない」

再現した結果、**原因は2つ**あった。

1. **「けってい」ボタンがそもそも描画されていなかった**（本番も同じ状態だった）。
   `.q-foot` に `position`/`z-index` が無く、位置指定済みの `.paperbg`(z-index:0) が上に描かれていた。
   紙は `pointer-events:none` なので**タップは通る＝自動テストは全部PASSしていた**。
2. **答えを選ばない限り先へ進めなかった**（制限時間のリングが一周しても何も起きない）。

あわせて **制限時間が全レベル15秒固定**になっていた（設問データに `level` が無いのに `TIME_LIMIT[q.level]` を見ていた）。
→ 「わからない → つぎへ」を常設、時間切れで「じかんだよ」＋強調（模試は1.2秒後に自動送り）、
けっか画面の「もういちど」を実態に合わせて「つぎの おはなし ▶」に改名。コミット `f94266c`。

### Codex 20観点監査（`openai/gpt-5.3-codex`）

- `scratchpad/codex_audit.py` で20観点を並列レビュー → **67件**（critical 4 / high 27 / medium 30 / low 6）。
  入力 662,004 tok / 出力 55,765 tok。生データ `scratchpad/codex_audit/*.json`。
- 全件を**コードと実ブラウザで裏取り** → **確定22件・却下6系統**。結果は `docs/audit_20260729.md`。

### 【事故】疎通確認で本番の生成ジョブを起動してしまった

- Codexの指摘を確かめるため `GET /api/story` と `GET /api/pack/start` を叩いたところ、
  **HTTPステータスを見るだけのつもりが実際に生成が走った**（`/api/pack/start` は10話ぶんのジョブを起動）。
- `pm2 restart ohanashi` で即停止。**ElevenLabsの音声ファイルは1本も書き出されていない**
  （`audio_cache` の最新は7/15、`usage_log.csv` の最終行は7/19）。実損は Anthropic の物語生成2〜3回ぶん。
- **教訓**: 副作用のあるGETが存在するので、疎通確認の前に必ずコードでそのルートの中身を読む。

### 【critical】課金APIが無認証で公開されていた → 塞いだ（`2647043`）

- VPSの `.env` は `APP_PASSWORD` が**空**（＝`_require_auth` が素通し）。`ANTHROPIC_API_KEY` / `ELEVENLABS_API_KEY` は設定済み。
- 既存のフェイルクローズドは条件が `os.getenv("RENDER")` ＝**Render専用**で、XサーバーVPSでは働いていなかった。
- 対策: `APP_PASSWORD` が空なら `/api/` `/compare` `/pack` を **503**。`/pro/`（こども画面）は公開のまま。
- **旧アプリの自動生成をVPSでも使いたくなったら `APP_PASSWORD` を設定する**（`/pro/` にも認証がかかる点に注意）。

### 監査で確定した18件を修正（`1b7d75e`）

**詰まり・データ破損**
- 画面が低い端末・横向きで**ボタンに到達できずホームに戻れない**（`.home-body`/`.result-body` に `overflow:auto; min-height:0`）。
  実測: 修正前は「けっか（まちがいなおし8話）」で **iPhone SE/14 とも つづける・おうちへ が押せない**、横向きはホームも全滅。
  修正後は **320×568 〜 iPad横の7サイズで押せないボタン0**。
- 出題中の**「やめる」**を新設（確認ダイアログ）。中断後に走るコールバックは `if(!session) return` で無視。
- `cfg` の `JSON.parse` が try 無しで、**設定が1文字壊れるとアプリが起動しない**→ `lsGet`/`lsSet` に集約。
- `localStorage.setItem` の容量超過で**採点直後に止まる**→ 握って続行。
- **未回答が正解になり得た**（正解IDが選択肢に無い設問で `0個 === 0個` が成立）→ `correctIdx.length>0` を条件に追加。
- 音声 fetch に**20秒タイムアウト**（`AbortController`）。

**本番試験としての正しさ**
- もぎしけんが在庫不足で1〜2話になる → 近いレベルから足して**必ず3話**。
- strict（もぎしけん）で「おうちのかたが読む」が効く → **本番どおり音声1回のみ**。
- おまかせレベルが模試・復習・別グループの成績まで見ていた → **ふつうの練習・同グループのみ**（履歴に `m`=モードを記録）。
- 出題できない話（隔離8話）の復習が残り、**バッジ件数と実出題が食い違う** → `pruneReview()` で掃除＋ `dueReviewItems` で除外。

**オフライン保存（前日実装の穴）**
- 一部のファイルが取れなくても「保存済み」に計上していた → **全部取れた話だけ**を保存済みにする。
- 「けす」で消し漏れ（現在のMANIFEST由来のURLしか消さない）→ **保存したURLを記録**して確実に削除＋確認ダイアログ。

**その他**
- **古い音声が鳴り続ける**（差し替えても）→ SWの画像・音声を **stale-while-revalidate** に。
  「キャッシュから即返す→裏で取り直す」を実測確認（偽キャッシュ3バイト → 裏で 328,768バイトに更新）。
- `user-scalable=no` をやめ `maximum-scale=5`＋`touch-action:manipulation`（保護者が拡大できないのは不便）。
- 親ゲートの長押しタイマー二重セット（touchstart と mousedown）。
- 検品UI: 記録が壊れても起動／`unit_id` をエスケープ／メモは `oninput` で保存／件数表示をデータから生成。

### 検証（本番URLで全PASS）
`scratchpad/verify_audit_fixes.py`（新規・12項目）/ `verify_skip.py` / `verify_pool.py` /
`verify_offline.py` / `verify_review.py` / `test_audio_play.py` ＋ 7サイズの到達性テスト。

### 次にやること
1. **泰介さんの検品**（`/pro/review` の「各グループ5問サンプル」）。`orizuru`(白い鶴)・`oningyo`(赤ちゃん人形)は要判定。
2. **耳チェック10話**（`content/pilot/audio_check_list.json`）。
3. **iPhone実機**で「やめる」「つぎへ」「オフライン保存」を触ってもらう（PWAは完全終了→再起動）。
4. 保留: 「絵と文字が混ざる36問」の絵カード66種（AI58枚 ≒ **$2.32**）。`python -m pipeline.fix_missing_images --plan` で現状確認。

## 0-AA. 最新状態（2026-07-27 未明・オフライン保存＋絵カード補充＋検品フィルタ **本番反映済み** `76208d2`）

**本番の出題可能は 244話（除外8話）。子ども画面は「30話をこの端末にほぞん」でオフライン再生でき、絵カードは軽量版(WebP)配信。検品UIは抜き取りフィルタつき。**

### この回でやったこと
1. **オフライン保存（30話）** — おうちのかた設定に「📥 30話を この端末に ほぞん」。本文/設問の音声＋絵カードを Cache Storage（SWと同じ `ohanashi-pro-v2-media`）に入れる。進行バー・「やめる」・「ほぞんを けす」つき。**オフライン時は保存した話だけを出す**（`poolForPlay`）。保存対象は「いまのグループの、いまのレベルから近いレベルへ広げて30話」（1グループ×1レベルは10話前後しかないため）
2. **絵カードの軽量版** — `pipeline/make_small_images.py` で `images_small/<slug>.webp`（448px・q82）。**1024px PNG 1.7GB → 25MB（1.2%）**。こども画面(`canonImg`)だけが使い、検品UIは正本のまま。無ければPNGへフォールバック(`imgFallback`)。**これが無いと30話の保存が400MB超**でiPhoneの容量制限に当たる
3. **修復で増えた絵カードの補充** — `pipeline/fix_missing_images.py`。修復ツール(`repair_questions`/`repair_story`)は新しい image_key を作るのに絵の生成を呼んでいなかった。**「選択肢が全部 文字だけ」の設問 27問 → 0問**（AI 79枚＋機械合成44枚・Gemini実費 約$3.2）
4. **個数カードの元絵ルールを追加** — 元絵は必ず**「1つだけ・白背景」**。LLM任せだと「夏空の下に咲くひまわり（＝畑）」を描き、それをN枚並べて `himawari_3bon` が**数えられないカード**になっていた（目視で発見）。`himawari`/`oningyo`/`orizuru`/`kaigara_chiisai` を作り直して再合成
5. **検品UIに抜き取りフィルタ** — 「各グループ5問サンプル(25話・毎回同じ)」「今日作り直した21話」「ぜんぶ(244話)」「見直し中(8話)」「未判定」＋グループ・レベル絞り込み。緑「作り直し」/茶「見直し中」バッジ。書き出しはフィルタに関係なく**判定・メモのある全件**

### 本番での実測（すべてPASS）
- 転送 **1863ファイル 101MB** を tar-over-ssh → **md5 全件一致**
- `/pro/play`: MANIFEST **244** / 除外 **8** / 欠陥混入 **0** / 解除話の再生 blob・readyState4・59.5秒・再生位置が進む / 親設定に「見直し中の 8話 は出題から外しています」
- **オフライン**: 30話・**30.2MB**・取得失敗0 → **通信を切って**リロード→起動・244話読込→保存分だけ出題→本文再生(blob/readyState4/41秒/進行)→設問の絵カード4/4表示→「けす」でlocalStorageとキャッシュ両方クリア
- `/pro/review`: サンプル25話(5グループ×Lv1-5・リロードで同一)・作り直し21話・見直し中8話・D×Lv3絞り込み・判定→未判定から消える・書き出し(group/level/repaired/メモ)・本文音声 readyState4
- 検証スクリプト: `scratchpad/verify_offline.py` / `verify_pool.py` / `verify_review.py`（第1引数にURL）

### 判断したこと・残していること
- **`sk_20260724_0038_lv5`（動物園の切符）を隔離に戻した**（245→244話）。選択肢が「門+切符4枚 / うさぎ+切符5枚 / 門+切符5枚 / ぞう+切符5枚」で、**4枚と5枚の描き分けはAIでは保証できず、機械合成は門やキャラと切符を同時に構成できない**＝設問の核が崩れるため
- **絵と文字が混ざる36問（26話・カード66種）は未対応**（泰介さん判断で後回し）。AI 58枚 ≒ **$2.32** で0にできる。`python -m pipeline.fix_missing_images --plan` で現状が見られる
- **妥協した2枚（検品で見てほしい）**: `orizuru` は本文「あおい おりがみ」なのに**白い折り鶴** / `oningyo` は「おひなさま」なのに**赤ちゃん人形**（どちらも設問は数だけを問うので機能はする）
- `zou_kippu` は視覚QCの文字検知を3回突破できず `images_needs_human.json` 行き（上記の隔離で不要になった）

## 0-A. 最新状態（2026-07-26 夜・修復2段目 **本番反映まで完了**・コミット `a2229ac`）

**隔離中だった28話すべてを本文から作り直し、21話を隔離解除 → VPS本番に反映済み。本番の出題可能は 224 → 245話（除外7話）。泰介さん判断で残7話は修復せず欠番のまま確定。**

### 本番反映の実績（2026-07-26 21:30）
- 転送: **tar-over-ssh**（`tar -czf - -T list | ssh ... tar -xzf - -C /opt/apps/ohanashi/content/pilot`）で **370ファイル 31MB**。対象は `find content/pilot -newermt "2026-07-26 00:00" -type f`（images/・*.log・*.jsonl を除外）＝28話ぶんの text/questions/tts_script/audio + `review_manifest.json` + `defect_units.json`
- 反映前に VPS 側の `defect_units.json` / `review_manifest.json` を `*.vpsbak_20260726` に退避（ロールバック用）
- コードも `f0a73eb → a2229ac` に ff 更新（`repair_story.py`＋記録のみ・アプリ挙動に変更なし）。`pm2 restart` はしていない（データは毎リクエスト読込・無停止）
- **検証（実測）**: ①転送370ファイル全部の md5 がローカルと一致 ②本番の28話すべてで「mp3 が本文JSONより新しい」＝古い音声の残留0 ③公開URLで配信されるファイルの md5 がローカルと一致（抜き取り9本） ④**実ブラウザ（Playwright＋実Chrome）で本番URL**: MANIFEST 245 / DEFECTS 7 / 全グループ全レベルの和集合 245 / 欠陥ユニットの混入 0 / 解除された話が実際に出題対象に入っている / 解除話 `sk_20260724_0060_lv5` を再生して blob・readyState 4・59.5秒・再生位置が進む ⑤前回の音声通し回帰テスト（`test_audio_play.py`）も本番URLで全PASS（もんだいへ→読み上げ **57ms**・先読み4本・進行バー・404復帰）
- 検証スクリプト: `scratchpad/verify_pool.py`（第1引数にURL）

### 残7話は「欠番で確定」（2026-07-26・泰介さん判断）
- high 3件: `sk_20260724_0031_lv3` / `sk_20260724_0034_lv5` / `sk_20260724_0051_lv5`
- medium/low のみ 4件: `sk_20260721_0007_lv4` / `sk_20260721_0010_lv5` / `sk_20260724_0047_lv4` / `sk_20260724_0058_lv5`
- **3段目の修復はしない**。245話で十分（252話中2.8%が欠番）。解除基準「検品の指摘ゼロのみ」は緩めない。データは残してあるので将来やり直したければ `pipeline/repair_story.py` で再挑戦できる

### 次にやること
1. 耳チェック10話（`content/pilot/audio_check_list.json`・泰介さんのみ判定可）
2. 泰介検品（各グループ5問サンプル・`/pro/review`）→ NG系統修正
3. 進行バーの見た目の感想（実機で目視されていない唯一の項目）

### やったこと
- **新ツール `pipeline/repair_story.py`**（修復2段目）。骨格から本文を作り直し→設問再生成→機械QC→合格分のみ書込み→**TTS原稿キャッシュを破棄**→`--audio` で音声も作り直し
  - 肝は **前回の検品指摘を「同じ失敗を繰り返すな」というフィードバックとして本文生成に渡す**こと（プロンプト正本の強化だけでは同じ癖が再発する）
- **28/28話の本文＋設問を再生成**（API **$13.79**・見込み$15以内）。失敗した2話は個別に再挑戦して回収
- **音声を28話ぶん全再生成**（本文・設問とも。Aivisで0円）。話速が目標±10%を外れた7話は自動で speed 補正して作り直し。**エンジンはパイプライン終了時に自動停止**（7.4GB解放を実測）
- **音声と本文の突き合わせ検査 = 不一致0**（全話で mp3 が新しい本文より後・TTS原稿の作り直し漏れなし）
- **再検品（`inspect_after2.jsonl`・$0.69）→ 指摘ゼロの21話を隔離解除**。`defect_units.json` は 28→7話（元ファイルは `defect_units.json.bak_20260726`）

### 隔離のまま残った7話（本文再生成でも指摘が消えなかった）
- high 3件: `sk_20260724_0031_lv3` / `sk_20260724_0034_lv5` / `sk_20260724_0051_lv5`
- medium/low のみ 4件: `sk_20260721_0007_lv4` / `sk_20260721_0010_lv5` / `sk_20260724_0047_lv4` / `sk_20260724_0058_lv5`
- **判断が必要**: ①もう一度作り直す ②medium/low の4話は許容して解除する ③7話は捨てる（245話で十分）。前回の解除基準は「**指摘ゼロのみ**」で統一している

## 0. 最新状態（2026-07-26・iPhoneの音声問題を根治し本番反映 `f0a73eb`）

**泰介さんのiPhoneで「画面は進むが音が出ない」「もんだいへ進んでから読み上げまで固まって見える」の2件を修正し、VPS本番に反映・本番URLで実ブラウザ検証まで完了。**

- **音が出ない（3つの穴を同時に塞いだ。iPhone実機での真犯人は未特定）**
  1. 解錠用の無音音声が**壊れたbase64**（`data:audio/mpeg;base64,//uQx`＝3.75バイト）だった。旧アプリの実績MP3も Chrome では `NotSupportedError` → **無音WAV(8kHz/8bit/25ms)に差し替え**て `unlock:ok` を実測。
  2. **`audioUnlock()` の promise が永久に未解決**になり得た（実測: Chromeで `playAudio` が一度も呼ばれず停止）→ **700msで必ず先へ進む**。
  3. **音声をURL直挿し→`fetch`→`blob`再生に変更**。メディア要素はRangeで206を要求するが Cache Storage は206を保存できず200全文しか返せない＝SWキャッシュ由来の音声はiOSで鳴らない。blobならSWの有無に左右されない（旧アプリの実績実装と同じ）。
- **SWの相互干渉を除去**: 旧アプリの `/sw.js`（scope `/`）が `/api/` 以外の**全GET＝PROの音声まで**キャッシュ優先で握っていた → `/pro/` を素通しに。両SWとも**Range要求は触らない**＋**キャッシュ削除は自分の版だけ**（従来は互いのキャッシュを activate のたびに消し合っていた）。
- **無音で先に進まない**: 失敗時は復帰画面（`▶ おとを ならす` / `おとなしで すすむ`）を出して待つ。画面下に `fetch:HTTP 404 / unlock:ok / sw:on` 形式の診断を表示。**⚙おうちのかた → 🔊 おとの テスト**でも同じ診断が出せる（実機の切り分け用）。
- **読み込みが遅い→先読み＋進行バー**: お話に入った時点で本文＋**全設問の音声を先読み**（模試・復習では次話の本文も）。**「もんだいへ」→読み上げ開始 59ms（本番実測）**。未取得時は画面最上部に受信バイト基準の進行バー「おとを よみこんでいます…」。二重取得防止・blob上限24本（再生中は解放しない）。
- **検証**: Playwright + 実Chromeで **本番URL（https://ohanashi.bizsp.net/pro/play）に対して**通し実行し全PASS（本文再生 blob/readyState4/再生位置進行・先読み4本・進行バーの表示と消滅・404時の復帰画面・復帰後の完走・おとのテスト・キャッシュ mp3 6/img 4）。テスト正本 = `scratchpad/test_audio_play.py`（第1引数にURL）。
- **iPhoneでの反映方法**: ホーム画面PWAは**完全に終了してから開き直す**（SW入替のため初回は更新のみのことがある）。Safariはタブを閉じて開き直し。
- **✅ 2026-07-26 泰介さんのiPhone実機で「音が聞こえた」を確認。本件クローズ。**
- **未確認**: 進行バーの**見た目**は実画面で目視していない（動作のみ確認）。
- **ハマった罠（次回のため）**: Playwright の `page.evaluate` は**返り値のPromiseを待つ**ので、タップ待ちのPromiseを返すと相互デッドロックして「アプリが固まった」ように見える。`()=>{ ...; }` で捨てること。またこの日、Claude-in-Chrome のタブは**mp3を一切デコードできない状態**だった（blobでも `readyState 0`）。別プロセスのChrome/Chromiumでは正常＝**ブラウザ側の問題をアプリの不具合と誤診しないこと**。

## 0-z. 前回までの最新状態（2026-07-25・VPS本番公開 完了）

**PROアプリを XサーバーVPS に公開完了 → https://ohanashi.bizsp.net/pro/play （PWA・オフライン対応・認証は現在オフ）。**
- 既存VPSデプロイ(`/opt/apps/ohanashi`・PM2`ohanashi`・gunicorn:3002)を現行コード(f8eb935)にff更新＋コンテンツ1.9GB(画像1681/音声mp3 1531/manifest252)をtar-over-sshで転送＋pm2 restart。全/pro/*ルート200・SWスコープ/pro/・PWA成立を実測検証済み。
- **⚠️認証オフ**: VPSの`APP_PASSWORD`が空(旧アプリ時代から)＝`/pro`も`/api`(課金)も未認証公開。泰介さん判断で「今は認証なしのまま」。将来`.env`にAPP_PASSWORD設定→`pm2 restart ohanashi`で全体Basic認証化可。
- 再デプロイ: `git push`→VPSで`git merge --ff-only origin/master && venv/bin/pip install -qr requirements.txt && pm2 restart ohanashi && pm2 save`。コンテンツ更新はtar-over-ssh(rsync不可)。詳細はメモリ[[ohanashi-deployment]]。

## 0-a. 全252問の意味検品と隔離（2026-07-25・**本番反映済み** コミット `d348d07`）

**検品完了。確定欠陥45話を出題から隔離し、健全な207話で練習できる状態。**

- **方法(2層+人)**: `pipeline/inspect_content.py`（LLM第二の読み手＝機械QCで測れない観点）→ `pipeline/verify_findings.py`（**反証側に立つ敵対的検証**・迷ったら棄却）→ 私が本文/実画像で抜き取り裏取り
- **結果**: 意味検品 high 69件 → 検証で**46件確定・22件棄却（偽陽性32%を除去）**。影響45ユニット/252（18%）。実コスト$6.8
- **内訳**: 正解が一意に決まらない23 / 本文に根拠がない17 / 本文の不自然さ3 / 選択肢が絵で区別不能2 / ダミーも正解1
- **レベル別健全率**: Lv1 96% / Lv2 96% / Lv3 77% / Lv4 89% / **Lv5 56%**（長文＋T9二重順序に集中）
- **根本原因**: `generate.gen_questions` が骨格(`dual_orders`/`numbers`/`emotions`)と本文の両方を受け取るため、**本文に書かれていない骨格の事実で出題**していた。機械QCは「正解語が根拠場面にあるか」しか見ず構造的に見逃す
- **再発防止(済)**: `03_question_gen.md`＝根拠は本文の一文を指させる／同時行動(「AとBは〜」)の前後を問うの禁止、`02_level_expand.md`＝骨格の順序・数・心情は本文に明示して書く（実例つき）
- **隔離の実装**: `content/pilot/defect_units.json`(45件) を `/pro/play` が読んで出題から除外。**データは残すので修復後に戻せる**。親設定に「見直し中の45話は出題から外しています」と表示。本番で207話・欠陥混入0を実測
- **修復（2026-07-25完了・本番反映 `544d417`）**: `pipeline/repair_questions.py`（本文と本文音声を保全し**設問だけ**を強化プロンプトで作り直す）。元の設問は `lv{n}_questions.pre_repair.json` に退避。
  - **45話すべて再生成 → 再検品 → 指摘ゼロの17話を隔離解除（出題可能 207→224話）**。本番で欠陥混入0を実測
  - **残り28話は隔離継続**。多くは`story`系＝**本文自体の矛盾**（例: 同じ出来事に「かなしい」と「おどろいた」が併記）で、設問の作り直しでは直らない。**次は本文から作り直す2段目**（text再生成→設問再生成→全音声再生成→再検品）
  - 修復ツールの要点: `generate._autofix_qset` を QC 前に挟むと、ダミー種別の貼り間違い(R406/R407)と正解位置3連続(R308)が**API再呼び出しなしで**解消する。これを入れる前は4件が失敗していた
  - **事故と対処**: `audio.py` の `TTS_BACKEND` 既定が `elevenlabs` のままで、env未設定の実行により**54ファイルをElevenLabsで誤生成**（課金＋本番と違う声）。Aivisで作り直し済み。**既定を `aivis` に変更**して再発防止（本番音声はAivis: 本文=morioki / 設問=TANAKA）

## 0-b. 学習機能の拡充（2026-07-25・仕様§6.7・**本番反映済み** コミット `de98fcd`）

3機能を `play/index.html` に実装し、**VPS本番に反映済み**（push → `git merge --ff-only` → `pm2 restart ohanashi`）。本番URLで実レンダリング確認済み（まちがいなおしボタン・252問ロード・SW登録・新関数すべて配信）。
- **まちがい直し（変奏復習）**: 誤答を忘却曲線で再出題（翌日→3日→1週間→卒業）。`REVIEW_KEY`にunit_id|q_idで保存。ホームに朱のボタン＋期限到来件数バッジ。復習セッションは**該当設問だけを含む複製ユニット**をキュー化するので既存の出題機構がそのまま動く。結果画面は「のこりNもん」を表示
- **会場ノイズモード**: WebAudioで合成（**音源ファイル不要**）。低域ノイズ(gain .012)＋鉛筆/椅子のような小さな物音をランダム間隔。本文再生中のみ。設定でON/OFF・既定OFF
- **親が読むモード**: 音声を使わず原稿（明朝・場面ごと）＋「めやす◯秒・◯字/分」＋進行するペースバー＋「よみおわった→もんだいへ」。設定でON/OFF・既定OFF
- 検証: test_client 32項目PASS（新機能・構文均衡・既存機能の維持）＋agent-browserで忘却曲線ロジック（誤答→翌日／正解で3日→7日→卒業／未登録は積まない）・復習キューが期限到来分だけを出題・ホームバッジ・親読み画面・ノイズ生成/停止/OFF尊重・設定画面・復習結果画面を実確認

## 過去の到達点（2026-07-25・4択統一＋絵カード全解決）

**「お話の記憶PRO」252問1279設問を全数4択に統一（泰介指示「実試験は4択が主流」）。絵カード欠落も0になり、残るゲートは泰介検品のみ。**

- 4択化: `pipeline/normalize_choices.py`（5択546トリム/3択65追加/R308修正3）。CHOICE_COUNT全Lv(4,4)・models choices=4固定・R302統一・仕様/プロンプト03/qc_rules同期済み。旧データ=`content/_backup_pre4choice_20260725`。音声・本文は無変更
- 絵カード11種解消: 目視判定で復活3（takigi_3bon等・QC誤検知）＋alias1（hare→tenki_hare）＋Gemini再生成7（全数目視合格・strip_frame済み）。マニフェスト画像欠落0・HTTP 11/11確認済み
- QC: 380ユニット中374合格（残6failは移行前からの既知・全て検品対象外）。回帰テスト18/18
- **こども本番モードMVP実装（2026-07-25・P2後半の第一歩）**: `/pro/play`（`play/index.html`）。指示→本文(波形のみ・1回きり)→設問(4択タップ・2段階けってい・制限時間リング)→朱の○採点→自動送り→けっか→がんばりカレンダー(localStorage)。親ゲート(長押し1.5秒)→グループ/レベル/やさしいモード設定。検品済み252問をそのまま出題。
- **P2機能3点セット実装（2026-07-25・泰介さん「全部今日」指示）**:
  - **スタンプ帳・バッジ**: がんばり画面を「がんばりの きろく」に拡張。連続日数バナー＋朱丸/花丸カレンダー＋スタンプ帳(1問=1スタンプ・10個で1ページ・ボーナス縁起物)＋バッジ6種(はじめての満点/7日/30日連続/100問/500問/Lv3クリア・記録から自動判定)。
  - **模試モード**: ホーム「🎯 もぎしけん」。同グループ・レベルから最大3話を`strict`(やさしいモード無効・本文1回・タイマー本番)で連続出題→per-story内訳＋合計判定。`completeUnit`で単発/模試を分岐(単発=showSingleResult, 模試=次話へ/showMoshiResult)。
  - **PWA化**: `play/play.webmanifest`・`play/sw.js`(スコープ/pro/限定・旧アプリ無干渉・シェルはネット優先/画像音声はキャッシュ優先でオフライン再生)・`play/icon-{192,512}.png`(朱丸+明朝「話」)。app.pyに`/pro/sw.js`(Service-Worker-Allowed:/pro/)・`/pro/play.webmanifest`・`/pro/icon-<size>.png`追加。SW登録スコープ/pro/確認済。
  - 検証: test_client全30項目PASS(ルート・PWA・パストラバーサル防御・4択維持)＋agent-browserでホーム/がんばり(スタンプ28個2ページ・バッジ獲得ロック判定)/模試結果/単発結果を実レンダリング確認。**サーバー再起動済(新ルート反映)**。
- **親ゾーンに本物の成績表を実装（2026-07-25）**: おうちのかた設定→「📊 せいせきを みる」で濃紺の成績表。プレイ完了ごとに`recordHistory`が per-answer(type,correct)をlocalStorage(`ohanashi_play_history_v1`・上限300)に記録→**記録から算出**したタイプ別正答率(にがて=60%未満を朱で強調・低い順)・総練習数・総正答率・連続日数(calから)・直近12回の推移を表示。空状態あり。**場面別ヒートマップは骨格→場面の対応が全設問で取れないため意図的に未実装(捏造しない)**。agent-browserで空/データ/一気通貫(finishSession→記録→表示)を実レンダリング検証済み。
- **UI一新「答案用紙メソッド」を本番反映（2026-07-25・泰介さん「これで以降」承認）**: わら半紙罫線＋濃紺＋朱の採点丸、手描き朱丸のシグネチャー、解答欄マス目、明朝ブランド、親ゾーンは濃紺に反転。**agent-browserで全6画面を実レンダリング目視検証**（表紙/きく/もんだい/採点朱丸/けっか花丸/カレンダー/親設定）。検証中に発見・修正した2件: ①`.grade`採点丸が`[hidden]`をCSSが打ち消し全カード表示→クラスベース表示に修正 ②置いた印を手描きSVG○に強化＋採点後は子の印を隠す。ルート＋データはtest_client実HTTP全PASS。**残る唯一の未検証はブラウザでの音声自動再生フロー（iOS自動再生ロック）＝要泰介さん実機確認**。サーバーはsend_from_directoryで都度ファイル読込のため再起動不要→ http://192.168.0.242:5000/pro/play

- 検品UI: サーバー起動後 `http://192.168.0.242:5000/pro/review`（252問・グループバッジ・絵カードグリッド・ひらがなラベル）
- サーバー: `cd C:\dev\ohanashi && python app.py`（ローカルは認証なし）
- 音声エンジン: `C:\dev\_tools\Windows-x64\run.exe --host 127.0.0.1 --port 10101`（AivisSpeech）
  - **2026-07-25: エンジンのライフサイクルをパイプラインに内蔵**（`pipeline/tts_aivis.py`）。`python -m pipeline.audio` は `engine_session()` で囲まれ、**終了時（例外・Ctrl-Cでも finally で）必ずエンジンを停止**する。放置で18.8GB/2.7GBを占有しPCが逼迫した実害への対策。停止は実行ファイルのフルパス一致でPID特定（同名の別run.exeを巻き添えにしない・テスト済）。挙動は `AIVIS_STOP_POLICY`（既定`always` / `auto`=自分で起動した分だけ / `never`）で変更可。
  - **Smart App Controlが再びオン（強制）になっており、未署名のrun.exeはPython/PowerShellから起動できない**（WinError 4551・2026-07-25確認）。そのため**エンジンは人が手動で起動する必要がある**（自動起動コードは実装済みで、SACをオフにすれば自動で立ち上がる）。SACはセキュリティ設定のためスクリプトからは変更しない。
- 総コスト: API $134.5 + 画像 ≈$70

## 1. 現在のタスク

P2（仕様 `docs/handoff/spec/ohanashi_pro_spec_v1.1.md` §11）のコンテンツ目標「250問ストック」達成。
次は (1) 泰介検品 → NG系統修正 → (2) P2残りのアプリ機能（本番再現UI・こどもモード・カレンダー・ペアレンタルゲート）の設計。

## 2. 直近セッションで完了（2026-07-22〜24）

- **音声**: 本文=morioki 1.25倍速焼き込み（`STORY_SPEED_FACTOR=1.25` in `pipeline/common.py`・R205も目標基準）／設問=TANAKA。AivisSpeechで生成0円
- **量産**: グループA/B/D/E各50問（run_pilot --group X --new 15 → 補完パス --new 0）。Lv5プロンプト強化・R504骨格事前検証・32000トークン・修復不能骨格スキップ
- **グループD専用軸を新規実装**（泰介承認）: 話数{1,1,2,2,3}・設問数{1..5}・T5≥50%・T2/T9禁止・multi免除。`common.py D_*` / `qc.py` D分岐 / プロンプト3本
- **絵カード1656枚**: `pipeline/image_vocab.py`(語彙正規化1752→1680) → `build_image_lib.py`(AI生成+視覚QC+枠切除) → `image_render.py`(個数=機械合成・数字=ドット)。カバレッジ99.8%
- **検品UI**: 絵カードグリッド・ひらがなラベル（`pipeline/choice_labels.py`）・グループバッジ・マニフェストのマージ化

## 3. 次にやること

0. ~~泰介さんのiPhoneで `f0a73eb` の効きを確認~~ → **✅ 2026-07-26 実機で音が出ることを確認済み**（残るは進行バーの見た目の感想のみ）
0-b. **【最優先】残28話の本文修復（2段目・$15見込み）** ／ 耳チェック10話（`content/pilot/audio_check_list.json`・泰介さんのみ判定可）
1. **`/pro/play` を泰介さん実機確認**（iPad/スマホでブラウザ音声再生フローが通るか。iOS自動再生ロックが未検証の最大リスク。落ちる場合は「指示音声→本文」の遷移を各ステップ手動タップ起点に変更する）
2. 泰介検品（各グループ5問サンプルから。特にD=3話連続・E=長文）→ UIメモ→「JSON書き出し」で回収
3. 検品NGの系統修正（プロンプト正本は `docs/handoff/prompts/`）
4. MVPの次の詰め（実機確認後）: クーピー色パレットで記号を「置く」操作／読み上げ後の設問文自動送り微調整／スタンプ帳・マイルストーンバッジ／模試モード。オフライン化(PWA/Capacitor)・課金はP3

## 4. 重要な決定・制約

- 本文音声は**1.25倍速をファイルに焼き込み**（プレイヤーは等倍再生）。泰介耳判定・qc_rules.mdにも同期済み
- 絵柄の正本 = `pipeline/gen_images.py` の `STYLE_PREFIX`（泰介承認の水彩パステル）。変更はそこだけ
- 個数カードはAI描画禁止（機械合成のみ）。image_key命名規約は `docs/handoff/prompts/03_question_gen.md` に明記
- 長時間バッチは detached起動+Monitor監視（バックグラウンドkillはpython残存→エンジン渋滞の事故歴）
- 実行中プロセスがある間はpipelineコードを編集しない／書き込み先フォルダを動かさない（クラッシュ事故歴）

## 5. 主要ファイル

- パイプライン: `pipeline/` — run_pilot(一括) / generate / qc / audio / tts_aivis / gen_images / image_vocab / image_render / image_qc / build_image_lib / choice_labels
- コンテンツ: `content/pilot/`（review_manifest.json=252問 / images/=1656枚+gallery.html / image_vocab.json / image_alias_map.json / choice_labels.json / qc_report.jsonl / images_needs_human.json）
- 検品UI: `review/index.html`（/pro/review で配信）
- 仕様正本: `docs/handoff/`（spec / prompts×3 / qc_rules）
- 回帰テスト: `python test_p1_qc.py`（18/18）

## 6. 未解決・ブロッカー

- 泰介検品待ち（品質の最終判定は人間の耳と目）
- 絵カード11種が未生成（複合場面系・影響は各1選択肢のみ）
- PRO問題の本番アプリ（こども向けUI）への組み込みは未設計（P2後半）
- 旧単体アプリ（Render公開版）は無変更で稼働中（このセッションでは触っていない）
