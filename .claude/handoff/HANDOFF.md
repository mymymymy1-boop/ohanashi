...
0. **【最優先】泰介さんのiPhoneで `f0a73eb` の効きを確認**（https://ohanashi.bizsp.net/pro/play ・PWAは完全終了→開き直し）。まだ音が出なければ **⚙おうちのかた → 🔊 おとの テスト**の診断行（`unlock:… / sw:… / session:…`）をそのまま回収する。進行バーの見た目も同時に見てもらう
0-b. 残28話の本文修復（2段目・$15見込み）／耳チェック10話（`content/pilot/audio_check_list.json`）
