...
QUESTION_PATTERNS = [
    ...
    "登場人物の気持ち・表情を問う（うれしい・かなしい・おこった・びっくり等。選択肢は顔の絵文字）",
    "誰が何を食べた／飲んだか・好き嫌い（食べたい物／食べたくない物）",
    "登場人物の服装の特徴（帽子・くつ・かばんなどの色や種類）",
    "位置関係を問う（誰のとなり・間・前後・上下・左右）",
...
def build_prompt(level: str) -> str:
    ...
    patterns_text = "\n".join(f"  ・設問{i+1}は「{p}」のパターンで出す。" for i, p in enumerate(chosen))
...
- 季節・動物・家族・遠足・お買い物・お祭り・お手伝いなど、子どもに身近な題材。毎回ちがう題材・登場人物にする。
